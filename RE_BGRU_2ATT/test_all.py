...
def index(sentence, ne):
    for i in range(len(sentence)):
        if sentence[i] == ne:
            return i
    return -1
testfile = "./IE_data/test.txt"

def test_all():
    # If you retrain the model, please remember to change the path to your own model below:
    pathname = "./model/IE_GRU_model-500"

    wordembedding = np.load('./data/vec.npy')
    test_settings = network.Settings()
    test_settings.vocab_size = 34115
    ...
    with tf.Graph().as_default():
        ...
        with sess.as_default():
            ...
            word2id = {}
            f = open('./IE_data/vec.txt', encoding='utf-8')
            content = f.readline()
            ...
            relation2id = {}
            id2relation = {}
            f = open('./IE_data/relation2id.txt', 'r', encoding='utf-8')
            while True:
                content = f.readline()
                if content == '':
                    break
            ...
            fop = open(testfile, 'r', encoding='utf-8').read().splitlines()
            true_true=0
            true_false = 0
            false_true = 0
            false_false = 0
            for i in fop:
                ...
                print(sentence)
                relation = 0
                en1pos = sentence.index(ent1)
                if en1pos == -1:
                    en1pos = 0
                en2pos = sentence.index(ent2)
                if en2pos == -1:
                    en2pos = 0
                output = []
                ...
                top1_id = prob.argsort()[-1:][::-1]
                flag = True
                for n,rel_id in enumerate(top1_id):
                    t = id2relation[rel_id]
                    if en2pos-en1pos>70:
                        t = 'N'
                    if t!='N' and labelee!='N':
                        true_true += 1
                    elif t!='N' and labelee=='N':
                        true_false += 1
                    elif t=='N' and labelee=='N':
                        false_false += 1
                    else:
                        false_true +=1
            print("正确关系是N，预测也是N：",false_false)
            print("正确关系是I，预测是N：",false_true)
            print("正确关系是N，预测是I：",true_false)
            print("正确关系是I，预测也是I：",true_true)
            a1 = float(true_true)/(false_true+true_true)
            a2 = float(true_true)/(true_true+true_false)
            F = float(2*(a1*a2)/(a1+a2))
    ...
    with tf.Graph().as_default():
        ...
        with sess.as_default():
            ...
            while True:
                ...
                print(sentence)
                relation = 0
                en1pos = index(sentence,en1)
                if en1pos == -1:
                    en1pos = 0
                en2pos = index(sentence,en2)
                if en2pos == -1:
                    en2post = 0
                output = []
                # length of sentence is 70
