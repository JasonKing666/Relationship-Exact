...
def parse_args():
    parser = argparse.ArgumentParser(description='inital arguments')
    parser.add_argument('--relation2id', type=str, default='./IE_data/relation2id.txt', help="the relation between NE's")
    parser.add_argument('--word2vec', type=str, default='./origin_data/vec.txt',
                        help="word embeddings")
    parser.add_argument('--train_data', type=str, default='./origin_data/vec.txt',
                        help="word embeddings")
...
def init():
    ...
    #需要改一下， 根据一个句子做分词之后的词的数量决定
    fixlen = 70
    # max length of position embedding is 60 (-60~+60)
    maxlen = 60

    train_sen = {}  # {entity pair:[[[label1-sentence 1],[label1-sentence 2]...],[[label2-sentence 1],[label2-sentence 2]...]}
    #train_sen 实体对元组为key， value是一个四维数组， 对于同一实体对有不同的关系label（维度一），对于同一关系有不同的句子嵌入（维度二），
    # 句子嵌入中包含了两个实体position的嵌入，对于句子中的没一个词的位置做一次嵌入（维度三），嵌入的格式是[word2id, en1embed, en2embed]（维度四）
    train_ans = {}  # {entity pair:[label1,label2,...]} the label is one-hot vector

    print('reading train data...')
    f = open('./IE_data/train.txt', 'rb')
    ...
    while True:
        ...
        tup = (en1, en2)
        label_tag = 0 #由于表示当前元组对应label在train_sen中的index
        if tup not in train_sen:
            #如果实体元组没在train_sen 的keys中出现
            #就加入新当前元组作为新的key
            train_sen[tup] = []
            ...
            if temp == -1:
                #在 train_sen 对应的key的键中没找当前label
                train_ans[tup].append(label)
