    ...
    with tf.Graph().as_default():
        ...
        with sess.as_default():
            ...
            for one_epoch in range(settings.num_epochs):
                ...
                np.random.shuffle(temp_order)#训练的时候有shuffle
                for i in range(int(len(temp_order) / float(settings.big_num))):
                    ...
                    if current_step > 400 and current_step % 50 == 0:
                        print('saving model')
                        path = saver.save(sess, save_path + 'IE_GRU_model', global_step=current_step)
